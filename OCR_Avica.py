...
def install_requirements():
    requirements = ['easyocr', 'opencv-python', 'pyperclip', 'torch']
...
def ocr_silent_auto():
    ...
    for label, (x, y, w, h) in zones.items():
        ...
        if label == "ID": id_val = text
        else: pass_val = text

    if id_val and pass_val:
        sys.stdout.write(f"{id_val}\n")
        sys.stdout.write(f"{pass_val}\n")
        sys.stdout.flush()
        
        # حفظ المتغيرات لـ GitHub Actions
